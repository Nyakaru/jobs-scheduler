...
export default class JoiHelper {
  static validateSubmission(submission: object, schema: joi.ObjectSchema) {
  ...
  static handleError(errorDetails: joi.ValidationErrorItem[]) {
    const errorObject: any = { errorMessage: 'Validation error occurred, see error object for details' };

    errorDetails.forEach(({
      message, type,  context: { label },
    }) => {
      switch (type) {
