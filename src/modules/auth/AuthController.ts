...
import { IClientData } from '../../interfaces/schedulerInterface';
class AuthController {
   ...
   */
    static async addClient(clientId: Redis.KeyType, secretKey: string): Promise<string> {
        let secretId: string;
        const client: IClientData = await redis.fetchObject(clientId);
        if (!client) {
            redis.saveObject(clientId, { clientId, clientSecret: secretKey});
            secretId = secretKey;
        } else {
